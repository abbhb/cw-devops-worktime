...
- 根据关键词聚合查询任务与用户需求工作项并供用户选择
- 选择日期范围与本地日报文件夹，按 `YYYY-MM-DD(.md)` 匹配 Markdown 日报
- 支持配置“日报文件日期策略”，默认按文件名当天匹配，也可切换为“文件名对应前一天工时”
- 生成按月日历预览，缺失日报以红色标签提示
- 日报内容超过 200 字时会自动截断后再提交，避免接口校验失败
...
6. `manifest.json` 已固定扩展公钥，后续继续覆盖更新同一扩展包时可保持稳定的扩展 ID。

## 使用方式

1. 打开插件右上角“全局配置”，填写 `reviewer`、`firstReviewer`，按需调整默认工时参数与“日报文件日期策略”。
